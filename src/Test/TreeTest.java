...
public class TreeTest {
	...
	public void replace() {
		...
		} catch (TDAArbol.InvalidPositionException e2) {
	...
	public void parent() {
		...
		} catch (TDAArbol.BoundaryViolationException e) {
	...
	public void children() {
		...
		Position<Integer> p;
		int i = 1;
		Queue<Position<Integer>> cola = new ColaConArregloCircular<>();
		try {
			p = T.root();
	...
	public void addBefore() {
		...
		try {
			...
			fail("createRoot() no deber�a lanzar la excepci�n InvalidOperationException para un �rbol vac�o.");
		} catch (EmptyListException e1){fail("los m�todos first() o last() no deber�a lanzar la excepci�n EmptyListException para una Lista con elementos [�PROBLEMA EN TDALISTA]");} catch (
				TDALista.InvalidPositionException e) {
	...
	public void removeInternal() {
		...
		hijo= T.children(T.root()).iterator().next();
		 ...
		 catch (TDAArbol.InvalidPositionException e){fail("children() no deber�a lanzar una excepci�n InvalidPositionException para una posici�n v�lida");}
		try{
		...
		}catch(TDAArbol.InvalidPositionException e){fail("El m�todo removeInternalNode no deber�a lanzar esta excepci�n al intentar eliminar la ra�z de un �rbol cuando esta tiene un s�lo hijo.");
	    }catch(TDAArbol.BoundaryViolationException e){}
		// Raiz sin hijos.
		try {
			T = getTree();
		...
		try {
			...
			fail("root() no deber�a lanzar la excepci�n EmptyTreeException para un �rbol no vac�o.");
				...
				TDALista.BoundaryViolationException e) {
			throw new RuntimeException(e);
